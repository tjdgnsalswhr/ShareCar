...
1. 사용자는 원하는 차량을 선택하여 예약한다. (사용할 수 없는 시간대는 선택할 수 없도록 되어있다.) 
2. 사용자는 결제를 진행한다.
3. 예약이 신청되면 시스템에서는 예약 정보가 생성된다.
5. 사용자는 예약을 취소할 수 있다.
6. 예약이 취소되면 시스템에서도 취소 처리가 된다.
7. 예약을 취소되면 예약 정보는 삭제 상태로 갱신 된다.
8. 고객이 예약 내역을 중간 조회한다.

#### 비기능적 요구사항

1. 트랜잭션
   - 예약이 결제까지 끝나지 않고 중간에 취소되면, 예약 정보는 아예 생성되지 않아야 한다. `Sync 호출`
...
![image](https://user-images.githubusercontent.com/32426312/130359343-ab899cc6-11f0-4f6e-9602-698ce6e296d8.png)
